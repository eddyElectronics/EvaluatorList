...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Update approval request body:', JSON.stringify(body, null, 2));
    
    const response = await fetch(`${API_BASE_URL}/updateApproval`, {
      method: 'POST',
      headers: {
    ...
    return NextResponse.json(
      { success: false, message: `Failed to update approval: ${(error as Error).message}` },
      { status: 500 }
    );
  }
}
